'''
Testing that includes tests for functionality that just doesn't work yet.
For example, correctly handling morphology and complex language rules
...
class TestUserVocabDatabase(unittest.TestCase):
    ...
    def test_coll_word_examples_nonlatin_alphabet(self):
        
        self.db.curr_list_id = 3
        expected_result =[("أزرق","السماء الزرقاء الصافية جميلة."),("الأصفر","الشمس صفراء;الموز الأصفر"),("أحمر","الدم أحمر لا أزرق;الورود هي حمراءالبنفسج هي زرقاء")] 
        self.assertEqual(self.db.coll_word_examples(),expected_result)
    ...
    def test_rem_word_from_sentence_futureversions_morphology(self):
        self.db.curr_list_id = 1
        wordexample_tuple_list = self.db.coll_word_examples()
        word_example_list = wordlist_manager.prep_fill_in_the_blank(wordexample_tuple_list)
        expected_result = [("Haus",["Das ____ hat drei Schlafzimmer und zwei Badezimmer, perfekt für meine Familie"," Er wollte sein ____ verkaufen weil es, ohne seine Kinder, zu groß war."]),("Frühstück",["Mein Magen morgens mag kein _________"," Zum _________ findet sie Pfannkuchen mit Ahornsirup am besten."]),("Büro",["Meine Mama ist gerade im ____ weil sie einen Termin mit ihren Arbeitskollegen hat"," Im ____ liegen alle meine Arbeitsunterlagen."]),("Der Fuß",["Sie haben auf meinen ___ getreten!"," Kannst Du meinem ___ eine Massage geben?"," Meine ____ sind zu groß für solche Schuhe."])]
