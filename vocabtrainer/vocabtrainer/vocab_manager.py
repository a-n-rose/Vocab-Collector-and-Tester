...
from vocabtrainer.input_manager import (
    rem_space_specialchar, 
    get_word_info, 
    get_list_info,
)
from vocabtrainer.wordlist_manager import (
...
# Set up the user's account: logging in or registering
    ...
    def get_username(self):
        ...
        if 'exit' == username.lower():
            raise vt.errors.ExitApp("Aw man. I didn't even catch your name. Come back soon!") 
        if username:
            return username
        else:
    ...
    def login(self,username):
        ...
        if 'exit' == password.lower():
            raise vt.errors.ExitApp("Aw man. You're leaving already? Come back soon!") 
        if password:
...
# Actions the logged_in user can do:
    ...
    def action_list(self):
        if self.is_user == True:
            ...
            raise vt.errors.ExitApp("Good job learning words! Until the next time :)")
        return None
